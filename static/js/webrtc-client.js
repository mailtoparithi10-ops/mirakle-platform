...
class WebRTCClient {
    ...
    async handleMeetingJoined(data) {
        ...
        for (const participant of data.participants) {
            if (participant.user_id !== window.currentUserId) {
                await this.createPeerConnection(participant.user_id, participant.user_name, true);
            }
        }
    }
    ...
    async handleParticipantJoined(data) {
        ...
        if (data.user_id !== window.currentUserId) {
            await this.createPeerConnection(data.user_id, data.user_name, false);
        }
    ...
    handleRemoteStream(userId, userName, stream) {
        ...
        this.participants.set(userId, { userName, stream });
    }
